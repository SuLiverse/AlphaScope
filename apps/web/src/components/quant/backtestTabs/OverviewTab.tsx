/**
 * Backtesting overview tab — typed props (no bag-of-any).
 */
...
import type { BacktestResultData, PerformanceMetrics, StrategyInfo } from "../backtestTypes";
...
export interface OverviewTabProps {
  ...
  qsMetrics: Record<string, number | string> | null;
  qsAvailable: boolean;
  qsLoading: boolean;
  qsError: string;
  computeQuantStats: () => void | Promise<void>;
...
export function OverviewTab({
  ...
  const onPickStock = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
      <div className="mb-6 flex flex-wrap items-end gap-3">
        ...
        <div className="rounded-xl border border-white/5 bg-black/40 px-4 py-2.5">
          ...
          >
            {strategies.length === 0 && (
              <option value="">{strategiesLoading ? "加载策略..." : "暂无策略"}</option>
            )}
            {strategies.map((strategy) => (
              <option
                key={strategy.id || strategy.name}
                value={strategy.id || strategy.name}
                className="bg-[#0f0f15] text-neutral-200"
              >
                {strategy.name}
              </option>
            ))}
          ...
          <h3 className="mb-6 border-b border-white/5 pb-3 text-xs font-mono uppercase tracking-widest text-neutral-400">
            ...
            {result
              ? `· ${result.strategy_id} / ${result.symbol}${
                  result.summary?.data_source_label ? " · " + result.summary.data_source_label : ""
                }`
              : "· 待运行"}
          </h3>
              ...
              <StableChartContainer>
                <LineChart data={equityData}>
                  ...
                  <YAxis
                    stroke="#737373"
                    fontSize={11}
                    tickFormatter={(val: number) => `${(Number(val) / 1000).toFixed(0)}k`}
                  />
                  <Tooltip
                    contentStyle={{
          ...
          <div className="mb-6 rounded-2xl border border-white/5 bg-white/[0.03] p-5">
            ...
            {!qsAvailable && (
              <p className="mt-3 text-xs text-amber-400">
                quantstats 未安装,无法生成完整报告。可执行 <code className="font-mono">pip install quantstats</code>{" "}
                启用。
              </p>
            )}
