...
@pytest.mark.anyio
async def test_api_get_requires_token_by_default(monkeypatch):
    """除显式公开路径外，GET 也必须经过本地 token 鉴权。"""
    main = _reload_api_main(monkeypatch)
    transport = ASGITransport(app=main.app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
...
def test_openapi_exposes_local_token_authorization(monkeypatch):
    main = _reload_api_main(monkeypatch)
    schema = main.app.openapi()

    scheme = schema["components"]["securitySchemes"]["LocalApiToken"]
    assert scheme == {
        "type": "apiKey",
        "in": "header",
        "name": "X-AlphaScope-Local-Token",
        "description": "Local API token generated by AlphaScope at startup.",
    }
    assert schema["paths"]["/api/conversations"]["get"]["security"] == [{"LocalApiToken": []}]
    assert "security" not in schema["paths"]["/health"]["get"]


def test_openapi_does_not_require_token_in_explicit_open_mode(monkeypatch):
