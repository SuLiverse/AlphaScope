...
### 修复
- `ruff format backend/ frontend/ tests/` 全量重排 288 文件(+1412/−4408, 纯格式零逻辑变更; `ruff check` 保持通过, `ruff format --check` 433 文件全部就绪)。

### 验证
- 格式化后离线套件 **1777 passed, 5 skipped**(18m56s, 0 回归)。
- 验收核查: v1.9.45 六项安全修复(C1-C6)提交齐全; 工作树干净, HEAD 同步 origin/main。
